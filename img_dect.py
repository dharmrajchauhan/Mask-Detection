...
class image_detection(MainWindow):
    def stop_cam(self):
    ...
    def fileopen(self):
        global filenames
        filenames = []
        filenameofuser = QFileDialog.getOpenFileName(self, 'Please select Image file', '',"image files (*.JPEG *.jpg *.png)")
        print(filenameofuser)
        print(type(filenameofuser))
        filenames.append(filenameofuser[0])
        print(f'filenames:- {filenames}')
        self.ui.idect_error.setText(filenameofuser[0])
        filename  = self.ui.idect_error.text()
        return filename
    ...
    def display_image(self):
        ...
        if len(filename) == 0:
            filename = image_detection.fileopen(self)
        ...
        try:
            if frame.all() != None:
                ...
                if (self.logic == 1):
                    try:
                        ...
                        for i in range(0, detections.shape[2]):
                            ...
                            if confidence > 0.5:
                                ...
                                faces = ([image_detection.prepare(faces)])
                    ...
                    pixmap = QPixmap.fromImage(simg)
                    qPixmap = pixmap.scaled(640, 480, QtCore.Qt.KeepAspectRatio)
                    self.ui.idect_output.setPixmap(qPixmap)
                    self.ui.idect_output.setAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)                
                    self.ui.idect_error.setText("Detection work completely")
